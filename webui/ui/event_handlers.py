...
class EventHandlers:
    ...
    def _gen_data(self,prompt, text, infer_mode):
        """根据选择的参考音频名称和文本生成音频数据"""

        if infer_mode == "普通推理":
            sampling_rate, wav_data = self.tts.infer(prompt, text, None) # 普通推理
        else:
            sampling_rate, wav_data  = self.tts.infer_fast(prompt, text, None) # 批次推理
    ...
    def gen_single(self,prompt, text, infer_mode, progress=gr.Progress()):
        ...
        sampling_rate, wav_data = self._gen_data(prompt, text, infer_mode)
