...
class IndexTTS:
    ...
    def infer_fast(self, audio_prompt, text, output_path, verbose=False, silence_duration=0.3):
        ...
        if silence_duration > 0:
            silence = torch.zeros((1, silence_samples), dtype=torch.float32, device='cpu')
            final_wavs = []
            # 由于 chunk_size 可能导致 wavs 长度小于 latent_length，
            # 我们需要正确处理句子和静音的添加
            for i in range(len(wavs)):
                if i > 0:  # 在非第一个音频块前添加静音
    ...
    # 原始推理模式
    def infer(self, audio_prompt, text, output_path, verbose=False, silence_duration=0.3):
